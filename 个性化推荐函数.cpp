...
void Commodity_p::recommend()
{
	...
	while (t)
	{
		...
		cout << " \t\t\t\t\t\t\t ┌=========================================┐" << endl;
		cout << " \t\t\t\t\t\t\t │" << "  NO." << setw(2) << i << "  " << setw(20) << t->product_name << "            │" << endl;
		cout << " \t\t\t\t\t\t\t │-----------------------------------------│" << endl;
		cout << " \t\t\t\t\t\t\t │" << "      ￥" << setw(6) << t->sell_price << "             " << setw(5) << t->sales_volume << "人付款" << "   │" << endl;
		cout << " \t\t\t\t\t\t\t │" << "            " << dd << "         " << setw(6) << t->comment << "+条好评" << "   │" << endl;
		cout << " \t\t\t\t\t\t\t └-----------------------------------------┘" << endl;
		int q = 0;
			...
			case 1:
			{
				...
				while (u)
				{
					if (u->order_number_1 == ge)
					{
						...
						switch (y)
						{
						case 1:u->sales_volume = u->sales_volume + 1;  cout << "\t\t\t\t\t\t\t 下单成功！" << endl; break;
						case 2:u->shopping = u->shopping + 1; u->shopping_car = u->product_name;  cout << "\t\t\t\t\t\t\t 成功加入购物车" << endl; break;
						}
